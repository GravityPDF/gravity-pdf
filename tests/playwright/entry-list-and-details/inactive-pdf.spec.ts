...
test.describe('Inactive PDF', () => {
  ...
  test('Entry List', async ({
    ...
    await expect(page.getByRole('link', { name: 'View PDF' })).not.toBeAttached();

    await pdf.createPdf(form.id, "Active PDF");

    await pdf.navigateToEntryList(form.id)
    await expect(page.getByRole('link', { name: 'View PDF' })).toBeAttached();
  });
  ...
  test('Entry Detail', async ({
    ...
    await expect(page.getByRole('link', { name: 'View', exact: true })).not.toBeAttached();
    await expect(page.getByRole('link', { name: 'Download', exact: true })).not.toBeAttached();

    await pdf.createPdf(form.id, "Active PDF");

    await pdf.navigateToEntryDetail(entry.form_id, entry.id)
    await expect(page.getByRole('link', { name: 'View', exact: true })).toBeAttached();
    await expect(page.getByRole('link', { name: 'Download', exact: true })).toBeAttached();
  });
});
