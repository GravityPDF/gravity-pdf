...
use GFPDF\Controller\Controller_Form_Settings_Rest_Api;
use WP_REST_Request;
use WP_REST_Server;
use WP_UnitTestCase;
 ...
 * @group api
 */
class Test_Controller_Form_Settings_Rest_Api extends WP_UnitTestCase {
	...
	/**
	 * @var Controller_Form_Settings_Rest_Api
	 */
	...
	function set_up() {
		...
		$this->controller = new Controller_Form_Settings_Rest_Api( $gfpdf->options, $gfpdf->gform, $gfpdf->misc, $gfpdf->templates );
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gravity-pdf/v1/form/(?P<form>[\d]+)', $routes );
		$this->assertArrayHasKey( '/gravity-pdf/v1/form/(?P<form>[\d]+)/(?P<pdf>[a-fA-F0-9]{13})', $routes );
	}
	...
	public function test_context_param() {
		...
		$this->assertSame( [ 'edit' ], $data['endpoints'][0]['args']['context']['enum'] );
	...
	protected function test_for_pdf_settings( $args ): void {
		...
		$this->assertArrayHasKey( 'zadani_border_colour', $args );
	...
	public function test_get_item() {
		...
		$this->assertSame( '#1C2C1C', $data['zadani_border_colour'] );
	...
	public function test_create_item() {
		...
		$this->controller->reregister_routes( 'zadani' );
	...
	protected function check_template_pdf_data( $pdf, $data, $context, $links ) {
		...
		$this->assertSame( $pdf['zadani_border_colour'], $data['zadani_border_colour'] );
		$this->assertSame( $pdf['show_form_title'], $data['show_form_title'] ? 'Yes' : '' );
		$this->assertSame( $pdf['show_page_names'], $data['show_page_names'] ? 'Yes' : '' );
		$this->assertSame( $pdf['show_html'], $data['show_html'] ? 'Yes' : '' );
		$this->assertSame( $pdf['show_section_content'], $data['show_section_content'] ? 'Yes' : '' );
		$this->assertSame( $pdf['enable_conditional'], $data['enable_conditional'] ? 'Yes' : '' );
		$this->assertSame( $pdf['show_empty'], $data['show_empty'] ? 'Yes' : '' );
	...
	protected function check_common_pdf_data( $pdf, $data, $context, $links ) {
		...
		$this->assertEquals( $pdf['conditional'], $data['conditional'] );
		...
		$this->assertSame( $pdf['rtl'], $data['rtl'] ? 'Yes' : '' );

		$this->assertSame( $pdf['format'], $data['format'] );

		$this->assertSame( $pdf['security'], $data['security'] ? 'Yes' : '' );
		...
		$this->assertSame( $pdf['public_access'], $data['public_access'] ? 'Yes' : '' );
		$this->assertSame( $pdf['restrict_owner'], $data['restrict_owner'] ? 'Yes' : '' );
	...
	public function test_update_item() {
		...
		$this->assertSame( '#000000', $data['zadani_border_colour'] );

		unset( $original_data['filename'], $original_data['pdf_size'], $original_data['zadani_border_colour'] );
		unset( $data['filename'], $data['pdf_size'], $data['zadani_border_colour'] );
		...
		$this->assertSame( '#000000', $pdf['zadani_border_colour'] );
	}
	...
	public function test_delete_item() {
		$template_pdf_id = $this->factory->pdf->create( [ 'template' => 'zadani', 'header' => 'Custom Header' ] );
	...
	public function test_prepare_item() {
		...
		foreach ( [ 'edit' ] as $context ) {
			...
			$data   = $response->get_data();
			$fields = array_filter( $this->controller->get_fields_for_response( $request ), function ( $item ) {
	...
	public function test_input_validation_create() {
		...
		$this->assertSame( 'rest_invalid_param', $data['data']['details']['name']['code'] );
	...
	public function test_assign_template_for_schema() {
		$GLOBALS['wp']->query_vars['rest_route'] = '/' . $this->controller::NAMESPACE . $this->controller::API_BASE;
		$_POST['template']                       = 'focus-gravity';
	...
	public function test_dont_assign_template_for_schema() {
		/* Check we only match our specific endpoint */
		$GLOBALS['wp']->query_vars['rest_route'] = '/' . $this->controller::NAMESPACE;
		$_POST['template']                       = 'focus-gravity';
		$this->controller->register_routes();
		...
		/* Check the template-specific properties are included */
		$this->assertArrayNotHasKey( 'show_form_title', $schema['properties'] );
		$this->assertArrayNotHasKey( 'show_html', $schema['properties'] );
		$this->assertArrayNotHasKey( 'show_section_content', $schema['properties'] );
		$this->assertArrayNotHasKey( 'header', $schema['properties'] );
		$this->assertArrayNotHasKey( 'first_header', $schema['properties'] );
		$this->assertArrayNotHasKey( 'background_color', $schema['properties'] );
	}
}
