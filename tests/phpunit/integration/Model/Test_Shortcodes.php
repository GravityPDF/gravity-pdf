...
class Test_Shortcodes extends TestCase {
	...
	public function test_gravitypdf_shortcode() {
		...
		$this->assertSame( '', $this->model->process( [] ) );
	...
	public function test_gravitypdf_confirmation() {
		...
		$this->assertSame( 'data', $results[0] );
	...
	public function test_add_shortcode_attr() {
		...
		/* Setup our test data */
		$content = json_decode( trim( file_get_contents( PDF_PLUGIN_DIR . '/tools/phpunit/data/pdf/shortcode-data.json' ) ), true );
		...
		$this->assertSame( '[gravitypdf text="Get My PDF"]', $results['shortcode'] );
		$this->assertSame( ' text="Get My PDF"', $results['attr_raw'] );
		$this->assertSame( 'Get My PDF', $results['attr']['text'] );

		$results = $this->model->add_shortcode_attr( $code2, 'class', 'class_here' );
		$this->assertSame( '[gravitypdf id="1231241221" text="View PDF" type="view" class="class_here"]', $results['shortcode'] );
		$this->assertSame( ' id="1231241221" text="View PDF" type="view" class="class_here"', $results['attr_raw'] );
		$this->assertSame( 'class_here', $results['attr']['class'] );

		$results = $this->model->add_shortcode_attr( $code2, 'type', 'download' );
		$this->assertSame( '[gravitypdf id="1231241221" text="View PDF" type="download"]', $results['shortcode'] );
		$this->assertSame( ' id="1231241221" text="View PDF" type="download"', $results['attr_raw'] );
		$this->assertSame( 'download', $results['attr']['type'] );

		$results = $this->model->add_shortcode_attr( $code3, 'entry', '{entry_id}' );
		$this->assertSame( '[gravitypdf id="78454" classes="my-custom-class" entry="{entry_id}"]', $results['shortcode'] );
		$this->assertSame( ' id="78454" classes="my-custom-class" entry="{entry_id}"', $results['attr_raw'] );
		$this->assertSame( '{entry_id}', $results['attr']['entry'] );
	}
	...
	public function test_gravitypdf_redirect_confirmation() {
		...
		$this->assertSame( '[gravitypdf id="555ad84787d7e" entry="{entry_id}" raw="1"]', $_POST['_gform_setting_url'] );

		/* Check for viewing URL */
		$_POST['_gform_setting_url'] = '[gravitypdf id="555ad84787d7e" type="view"]';

		$this->model->gravitypdf_redirect_confirmation( [ 'id' => 1 ] );
		$this->assertSame( '[gravitypdf id="555ad84787d7e" type="view" entry="{entry_id}" raw="1"]', $_POST['_gform_setting_url'] );
	...
	public function test_get_shortcode_information() {
		...
		$this->assertSame( '[gravitypdf]', $shortcodes[0]['shortcode'] );
		$this->assertSame( '', $shortcodes[0]['attr_raw'] );
		$this->assertSame( [], $shortcodes[0]['attr'] );

		$this->assertSame( '[gravitypdf id="1231241221" text="View PDF" type="view"]', $shortcodes[1]['shortcode'] );
		$this->assertSame( ' id="1231241221" text="View PDF" type="view"', $shortcodes[1]['attr_raw'] );
		$this->assertSame( '1231241221', $shortcodes[1]['attr']['id'] );
		$this->assertSame( 'View PDF', $shortcodes[1]['attr']['text'] );
		$this->assertSame( 'view', $shortcodes[1]['attr']['type'] );

		$this->assertSame( '[gravitypdf id="78454" classes="my-custom-class" entry="2000"]', $shortcodes[2]['shortcode'] );
		$this->assertSame( ' id="78454" classes="my-custom-class" entry="2000"', $shortcodes[2]['attr_raw'] );
		$this->assertSame( '78454', $shortcodes[2]['attr']['id'] );
		$this->assertSame( 'my-custom-class', $shortcodes[2]['attr']['classes'] );
		$this->assertSame( '2000', $shortcodes[2]['attr']['entry'] );

		$this->assertSame( '[gravitypdf id="afawfjoa420204" classes="my-custom-class-2" entry="3000" text="View PDF" type="view"]', $shortcodes[3]['shortcode'] );
		$this->assertSame( ' id="afawfjoa420204" classes="my-custom-class-2" entry="3000" text="View PDF" type="view"', $shortcodes[3]['attr_raw'] );
		$this->assertSame( 'afawfjoa420204', $shortcodes[3]['attr']['id'] );
		$this->assertSame( 'my-custom-class-2', $shortcodes[3]['attr']['classes'] );
		$this->assertSame( '3000', $shortcodes[3]['attr']['entry'] );
		$this->assertSame( 'View PDF', $shortcodes[3]['attr']['text'] );
		$this->assertSame( 'view', $shortcodes[3]['attr']['type'] );
	}
