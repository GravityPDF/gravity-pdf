 ...
 * @group form-settings
 */
class Test_Form_Settings extends TestCase {
	...
	public function test_actions() {
		...
		/* standard actions */
		$this->assertSame( 5, has_action( 'admin_init', [ $this->controller, 'maybe_save_pdf_settings' ] ) );
	...
	public function test_filters() {
		...
		/* validation filters */
		$this->assertSame( 10, has_filter( 'gfpdf_form_settings', [ $this->model, 'validation_error' ] ) );
		$this->assertSame(
			10,
			has_filter(
	...
	public function test_maybe_save_pdf_settings() {
		...
		try {
			...
			$this->fail( 'Expected Exception on failed capability check was not thrown.' );
		} catch ( Exception $e ) {
	...
	public function test_process_list_view() {
		...
		$this->assertStringContainsString( '<form id="gfpdf_list_form" method="post">', $html );
	...
	public function test_show_edit_view() {
		...
		$this->assertStringContainsString( 'method="post" id="gfpdf_pdf_form"', $html );
	...
	public function test_process_submission() {
		...
		$this->assertSame( 'sanitizing', $pdf['status'] );
	 ...
	 * @since 4.0
	 */
	public function provider_strip_filename(): array {
		return [
	...
	public function test_decode_json() {
		...
		/* Test pass result */
		$this->assertSame( $json, $this->model->decode_json( $json, 'other' ) );
	}
	...
	public function test_register_notifications() {
		...
		$this->assertCount( 3, $wp_settings_fields[ $group ][ $group ][ $setting ]['args'][ $option_id ] );
		...
		$this->assertCount( 1, $wp_settings_fields[ $group ][ $group ][ $setting ]['args'][ $option_id ] );
	...
	public function test_register_template_group() {
		...
		$this->assertSame( 'test', $this->model->register_template_group( 'test' ) );
