...
class Test_Rest_Form_Settings extends Test_Rest {
	...
	public function test_get_items() {
		$this->gf_factory()->pdf->create_many( 5, [
			'template' => 'rubix',
		] );
	...
	public function test_get_items_for_entry() {
		$this->gf_factory()->pdf->create_many( 2, [
			'template' => 'rubix',
		] );

		$this->gf_factory()->pdf->create( [ 'active' => false ] );
		$this->gf_factory()->pdf->create( [
			'conditional'      => 1,
			'conditionalLogic' => [
				'actionType' => 'show',
				'logicType'  => 'all',
		...
		$entry_id = $this->gf_factory()->entry->create( [ 'form_id' => $this->form_id, '1' => 'something' ] );
		$request->set_query_params( [
			'entry' => $entry_id,
		] );
	...
	public function test_get_items_with_filters() {
		$this->gf_factory()->pdf->create( [ 'template' => 'rubix' ] );
		$this->gf_factory()->pdf->create( [ 'template' => 'focus-gravity' ] );
		$this->gf_factory()->pdf->create( [ 'template' => 'zadani' ] );
		$this->gf_factory()->pdf->create( [ 'template' => 'blank-slate' ] );
	...
	public function test_get_item() {
		...
		$template_pdf_id = $this->gf_factory()->pdf->create( $template_pdf_config );
		...
		$this->assertSame( $template_pdf_config['font_size'], $data['font_size'] );
	...
	protected function check_template_pdf_data( $pdf, $data, $context, $links ) {
		...
		if ( is_array( $data['custom_pdf_size'] ) ) {
			$this->assertSame( $pdf['custom_pdf_size'][0], $data['custom_pdf_size']['width'] );
			$this->assertSame( $pdf['custom_pdf_size'][1], $data['custom_pdf_size']['height'] );
			$this->assertSame(
	...
	protected function check_common_pdf_data( $pdf, $data, $context, $links ) {
		...
		$this->assertSame( $pdf['conditional'] === 'Yes', $data['conditional'] );

		$this->assertSame( $pdf['font'], $data['font'] );
		$this->assertSame( $pdf['font_size'], $data['font_size'] );
		...
		$this->assertSame( $pdf['image_dpi'], $data['image_dpi'] );
	...
	public function test_update_item() {
		$template_pdf_id = $this->gf_factory()->pdf->create( [
			'template'                          => 'rubix',
			'filename'                          => 'Filename-{Text:5}',
			'pdf_size'                          => 'LETTER',
			'rubix_container_background_colour' => '#1C2C1C',
	...
	public function test_delete_item() {
		$template_pdf_id = $this->gf_factory()->pdf->create( [ 'template' => 'rubix', 'header' => 'Custom Header' ] );
		$this->gf_factory()->pdf->create( [ 'template' => 'rubix' ] );
