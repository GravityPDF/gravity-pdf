...
class Test_Helper_Templates extends TestCase {
	...
	public function test_get_template_path() {
		$this->assertStringContainsString( '/wp-content/uploads/PDF_EXTENDED_TEMPLATES/', $this->templates->get_template_path() );
		$this->assertDirectoryExists( $this->templates->get_template_path() );
	}
	...
	public function test_get_template_url() {
		$this->assertStringContainsString( '/wp-content/uploads/PDF_EXTENDED_TEMPLATES/', $this->templates->get_template_url() );
	...
	public function test_maybe_add_template_compatibility_notice() {
		$this->assertSame( 'Template', $this->templates->maybe_add_template_compatibility_notice( 'Template', PDF_EXTENDED_VERSION ) );

		$version = (float) PDF_EXTENDED_VERSION + 1;
		$this->assertSame( 'Template (Requires Gravity PDF v' . $version . ')', $this->templates->maybe_add_template_compatibility_notice( 'Template', $version ) );
	}
	...
	public function test_get_template_path_by_id() {
		...
		$this->assertSame( PDF_PLUGIN_DIR . 'src/templates/zadani.php', $path );
		...
		$path = $this->templates->get_template_path_by_id( 'zadani' );
		$this->assertSame( $test_path, $path );
		...
		try {
			...
			$this->fail( 'Expected Exception on missing template path lookup was not thrown.' );
		} catch ( Exception $e ) {
			$this->assertSame( 'Could not find the template: doesnt-exist.php', $e->getMessage() );
		}
	}
	...
	public function test_get_template_info_by_id() {
		...
		$this->assertSame( 'zadani', $info['id'] );
		$this->assertSame( 'Zadani', $info['template'] );
		$this->assertSame( 'Core', $info['group'] );

		$info = $this->templates->get_template_info_by_id( 'rubix' );

		$this->assertSame( 'rubix', $info['id'] );
	}
	...
	public function test_get_template_files_by_id() {
		...
		$this->assertStringContainsString( 'PDF_EXTENDED_TEMPLATES/test.php', $files[0] );
		$this->assertStringContainsString( 'PDF_EXTENDED_TEMPLATES/config/test.php', $files[1] );
		$this->assertStringContainsString( 'PDF_EXTENDED_TEMPLATES/images/test.png', $files[2] );
		...
		try {
			$files = $this->templates->get_template_files_by_id( 'test' );
			$this->fail( 'Expected Exception on missing PDF template file was not thrown.' );
		} catch ( Exception $e ) {
			$this->assertSame( 'Could not find PDF template file', $e->getMessage() );
		}
	}
	...
	public function test_get_template_configuration() {
		...
		/* Test failure first */
		$this->assertInstanceOf( 'stdClass', $this->templates->get_config_class( 'test' ) );
	...
	public function test_get_template_headers() {
		...
		$this->assertSame( 'Zadani', $header['template'] );
		$this->assertArrayHasKey( 'version', $header );
		$this->assertArrayHasKey( 'description', $header );
		$this->assertArrayHasKey( 'author', $header );
		$this->assertArrayHasKey( 'author uri', $header );
		$this->assertSame( 'Core', $header['group'] );
		$this->assertSame( '4.0-alpha', $header['required_pdf_version'] );
	...
	public function test_human_readable( $expected, $name ) {
		$this->assertSame( $expected, $this->templates->human_readable_template_name( $name ) );
	}
	 ...
	 * @since  4.1
	 */
	public function provider_human_readable(): array {
		return [
	...
	public function test_get_template_image() {
		...
		$this->assertSame( $test_path, $this->templates->get_template_image( 'zadani' ) );
