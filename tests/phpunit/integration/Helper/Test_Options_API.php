...
class Test_Options_API extends TestCase {
	...
	public function test_get_settings() {
		...
		$this->assertSame( 'custom', $settings['default_pdf_size'] );
		$this->assertSame( 'Awesomeness', $settings['default_template'] );
		$this->assertSame( 'dejavusans', $settings['default_font_type'] );
		$this->assertSame( 'No', $settings['default_rtl'] );
		$this->assertSame( 'View', $settings['default_action'] );
		$this->assertSame( 'No', $settings['default_restrict_owner'] );
		$this->assertSame( '20', $settings['logged_out_timeout'] );

		$this->assertIsArray( $settings['default_custom_pdf_size'] );
		$this->assertIsArray( $settings['admin_capabilities'] );
		...
		$this->assertSame( [ 'testing' ], $this->options->get_settings() );
	}
	...
	public function test_update_settings() {
		...
		$settings = $this->options->get_settings();
		$this->assertSame( 'Letter', $settings['default_pdf_size'] );
	...
	public function test_register_settings() {
		...
		$this->assertSame( 'Test Item', $wp_settings_fields['gfpdf_settings_general']['gfpdf_settings_general']['gfpdf_settings[my_test_item]']['title'] );
	...
	public function test_update_registered_field() {
		...
		/* Run false test */
		$this->assertCount( 0, $wp_settings_fields[ $group ][ $group ][ $setting ]['args'][ $option_id ] );

		/* Run valid test */
		$this->options->update_registered_field( 'form_settings', 'notification', 'options', 'working' );

		$this->assertSame( 'working', $wp_settings_fields[ $group ][ $group ][ $setting ]['args'][ $option_id ] );
	}
	...
	public function test_get_registered_settings() {
		...
		$this->assertSame( 'General Settings', $items['general'] );
		$this->assertSame( 'General Security Settings', $items['general_security'] );
		$this->assertSame( 'Extension Settings', $items['extensions'] );
		$this->assertSame( 'License Settings', $items['licenses'] );
		$this->assertSame( 'Tools Settings', $items['tools'] );
		$this->assertSame( 'PDF Form Settings', $items['form_settings'] );
		$this->assertSame( 'PDF Form Settings Appearance', $items['form_settings_appearance'] );
		$this->assertSame( 'PDF Form Settings Advanced', $items['form_settings_advanced'] );
	...
	public function test_get_form_pdfs() {
		...
		$this->assertSame( 'Gravity Forms Style', $pdf['template'] );
		$this->assertIsArray( $pdf['notification'] );
		$this->assertCount( 2, $pdf['notification'] );
		$this->assertSame( 'test', $pdf['filename'] );
		$this->assertIsArray( $pdf['conditionalLogic'] );
		$this->assertCount( 3, $pdf['conditionalLogic'] );
		$this->assertSame( 'custom', $pdf['pdf_size'] );
		$this->assertSame( '150', $pdf['custom_pdf_size'][0] );
		$this->assertSame( '300', $pdf['custom_pdf_size'][1] );
		$this->assertSame( 'millimeters', $pdf['custom_pdf_size'][2] );
		$this->assertSame( 'landscape', $pdf['orientation'] );
		$this->assertSame( 'dejavusans', $pdf['font'] );
		$this->assertSame( 'No', $pdf['rtl'] );
		$this->assertSame( 'Standard', $pdf['format'] );
		$this->assertSame( 'Yes', $pdf['security'] );
		$this->assertSame( 'my password', $pdf['password'] );
		$this->assertIsArray( $pdf['privileges'] );
		$this->assertCount( 8, $pdf['privileges'] );
		$this->assertSame( '300', $pdf['image_dpi'] );
		$this->assertSame( 'No', $pdf['save'] );
		$this->assertSame( '555ad84787d7e', $pdf['id'] );
	...
	public function test_get_pdf() {
		...
		$this->assertSame( 'My First PDF Template (copy)', $pdf['name'] );

		$pdf = $this->options->get_pdf( $this->form_id, '556690c8d7f82' );
		$this->assertSame( 'Disable PDF Template', $pdf['name'] );
	...
	public function test_add_pdf() {
		...
		$this->assertSame( 'Added PDF', $pdf['name'] );
		$this->assertSame( 'default-template', $pdf['template'] );
	}
	...
	public function test_update_pdf() {
		...
		$this->assertSame( 'My New Name', $new_pdf['name'] );
		$this->assertFalse( $new_pdf['active'] );
		...
		$this->assertInstanceOf( \WP_Error::class, $has_pdf_deleted );
	}
	...
	public function test_delete_pdf() {
		...
		$this->assertTrue( $this->options->delete_pdf( $this->form_id, $pid ) );
		$this->assertInstanceOf( \WP_Error::class, $this->options->get_pdf( $this->form_id, $pid ) );
	...
	public function test_get_pdf_filter() {
		...
		$this->assertSame( 'main filter fired', $this->options->get_pdf( $this->form_id, '555ad84787d7e' ) );
		...
		/* check filter was triggered */
		$this->assertSame( 'ID filter fired', $this->options->get_pdf( $this->form_id, '555ad84787d7e' ) );
	}
	...
	public function test_add_pdf_filter() {
		...
		/* verify the results */
		$pdf = $this->options->get_pdf( $this->form_id, $id );
		$this->assertSame( 'Add Filter Fired', $pdf['name'] );
		...
		/* verify the results */
		$pdf = $this->options->get_pdf( $this->form_id, $id );
		$this->assertSame( 'ID Add Filter Fired', $pdf['name'] );
	}
	...
	public function test_update_pdf_filter() {
		...
		/* verify the results */
		$pdf = $this->options->get_pdf( $this->form_id, '555ad84787d7e' );
		$this->assertSame( 'Update Filter Fired', $pdf['name'] );
		...
		/* verify the results */
		$pdf = $this->options->get_pdf( $this->form_id, '555ad84787d7e' );
		$this->assertSame( 'ID Update Filter Fired', $pdf['name'] );
	}
	...
	public function test_get_option() {
		...
		$this->assertSame( 'custom', $this->options->get_option( 'default_pdf_size' ) );
		$this->assertSame( 'Awesomeness', $this->options->get_option( 'default_template' ) );
		$this->assertSame( 'No', $this->options->get_option( 'default_restrict_owner' ) );
		$this->assertIsArray( $this->options->get_option( 'admin_capabilities' ) );
		...
		$this->assertSame( 'New Value', $this->options->get_option( 'default_pdf_size' ) );
		...
		$this->assertSame( 'RTL', $this->options->get_option( 'default_rtl' ) );
	...
	public function test_update_option() {
		...
		$this->assertSame( 'new pdf size', $this->options->get_option( 'default_pdf_size' ) );
		...
		$this->assertTrue( $this->options->update_option( 'default_pdf_size', 'new pdf size' ) );
		$this->assertSame( 'filtered option', $this->options->get_option( 'default_pdf_size' ) );
		...
		$this->assertSame( 'filtered admin option', $this->options->get_option( 'default_restrict_owner' ) );
	...
	public function test_add_custom_fonts() {
		...
		$this->assertCount( 2, $get_fonts['Unicode'] );
		$this->assertCount( 2, $get_fonts['User-Defined Fonts'] );
	}
	...
	public function test_get_custom_fonts() {
		...
		$this->assertSame( 'hel', $get_fonts[0]['id'] );
		$this->assertSame( 'cal', $get_fonts[1]['id'] );
	}
	...
	public function test_get_privileges() {
		$this->assertIsArray( $this->options->get_privilages() );
	}
	...
	public function test_settings_sanitize() {
		...
		$this->assertSame( 'test', $this->options->settings_sanitize( 'test' ) );
		...
		$this->assertSame( 'custom', $initial_settings['default_pdf_size'] );
	...
	public function test_sanitize_trim_field( $expected, $input ) {
		$this->assertSame( $expected, $this->options->sanitize_trim_field( $input ) );
	}
	...
	public function test_sanitize_all_fields( $type, $value, $expected ) {
		$this->assertSame( $expected, $this->options->sanitize_all_fields( $value, '', [], [ 'id' => $type, 'type' => $type ] ) );
	}
	 ...
	 * @since 4.0
	 */
	public function provider_sanitize_all_fields(): array {
		return [
			[
				'rich_editor',
	...
	public function test_sanitize_required_field( $type, $value, $expected ) {
		...
		/* Check the results */
		$this->assertSame( $expected, count( $wp_settings_errors ) );
	}
	 ...
	 * @since 4.0
	 */
	public function provider_sanitize_required_field(): array {
		return [
			[ 'select', [], 1 ],
			[ 'multicheck', [], 1 ],
			[ 'paper_size', [], 1 ],
	...
	public function test_get_form_value( $input, $expected ) {
		...
		$this->assertSame( $expected, $this->options->get_form_value( $input ) );
	}
	 ...
	 * @since 4.0
	 */
	public function provider_get_form_value(): array {
		return [
