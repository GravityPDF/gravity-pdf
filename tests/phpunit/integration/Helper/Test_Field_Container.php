 ...
 */
class Test_Field_Container extends TestCase {
	...
	public function test_row() {
		...
		$this->assertSame( '<div class="row-separator odd">', $this->generate( $field ) );

		/* Check it closes / opens correctly */
		$this->assertSame( '</div><div class="row-separator even">', $this->generate( $field ) );

		/* Check it now closes correctly */
		$this->assertSame( '</div>', $this->close() );
	}
	...
	public function test_two_columns() {
		...
		$this->assertSame( '', $this->generate( $field ) );
	...
	public function test_maybe_display_faux_column() {
		...
		$this->assertSame( '', $this->maybe_display_faux_column( $field ) );

		/* Add a column to a new row then see if we get a faux column returned */
		$field->cssClass = 'gf_left_third';
		$this->generate( $field );

		$this->assertStringContainsString( 'gfpdf-column-placeholder', $this->maybe_display_faux_column( $field ) );
	...
	public function test_helper_field_container_void() {
		...
		$this->assertSame( $total_methods, $overridden_methods, 'Helper_Field_Container_Void does not override all public methods of the parent object' );
	}
}
