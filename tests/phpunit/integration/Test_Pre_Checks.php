 ...
 * @group pre-checks
 */
class Test_Pre_Checks extends TestCase {
	...
	public function test_init() {
		...
		$this->assertSame( 10, has_action( 'plugins_loaded', [ $this->gravitypdf, 'plugins_loaded' ] ) );
	}
	...
	public function test_check_wordpress( $min_version, $test_wp_version, $expected ) {
		...
		/* run our test */
		$this->assertSame( $expected, $this->gravitypdf->is_compatible_wordpress_version() );
	}
	...
	public function test_check_gravityforms( $min_version, $test_gf_version, $expected ) {
		...
		/* run our test */
		$this->assertSame( $expected, $this->gravitypdf->check_gravity_forms() );
	}
	...
	public function test_convert_ini_memory( $memory, $bytes ) {
		$this->assertSame( $bytes, $this->gravitypdf->convert_ini_memory( $memory ) );
	}
	...
	public function test_get_ram( $memory, $bytes ) {
		...
		$this->assertSame( $expected_mb, $this->gravitypdf->get_ram( $memory ) );
	}
	...
	public function test_check_ram( $memory, $expected ) {
		$this->assertSame( $expected, $this->gravitypdf->check_ram( $memory ) );
	}
	...
	public function test_loader_notice() {
		...
		$this->assertSame( 10, has_action( 'admin_notices', [ $this->gravitypdf, 'display_notices' ] ) );
	}
	 ...
	 * @since 4.0
	 */
	public function provider_memory(): array {
		return [
			[ '60mb', 62914560 ],
			[ '60MB', 62914560 ],
			[ '60m', 62914560 ],
	 ...
	 * @since 4.0
	 */
	public function provider_memory_minimum(): array {
		return [
			[ '512M', true ],
			[ '100M', true ],
			[ '75M', true ],
	 ...
	 * @since 4.0
	 */
	public function provider_version(): array {
		return [
			[ '5.5.5', '5.5.6', true ],
			[ '5.5.5', '5.5.5', true ],
			[ '5.5.5', '5.5.5.0', true ],
