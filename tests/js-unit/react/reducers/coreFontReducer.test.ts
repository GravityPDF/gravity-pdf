...
import reducer, {
	initialState,
} from '../../../../src/assets/js/react/reducers/coreFontReducer';
...
describe('Reducers - coreFontReducer', () => {
	...
	describe('GET_FILES_FROM_GITHUB_SUCCESS', () => {
		test('check the correct state gets returned when this action runs', () => {
			const data = ['Font 1', 'Font 2'];
			const newData = ['Font 3', 'Font 4', 'Font 5'];
			newState = reducer(initialState, {
	...
	describe('REQUEST_SENT_COUNTER', () => {
		test('check the correct state gets returned when this action runs', () => {
			...
			expect(state.console.completed.message).toBe(
			...
			expect(newState.console.completed.message).toBe(
				'3 CORE FONT(S) DID NOT INSTALL CORRECTLY'
			);
