...
describe('Sagas - coreFonts', () => {
	describe('watchGetFilesFromGitHub()', () => {
		...
		test('should check the watcher to loads up the getFilesFromGitHub function and call GET_FILES_FROM_GITHUB action', () => {
			expect(gen.next().value).toEqual(
				takeLatest(GET_FILES_FROM_GITHUB, getFilesFromGitHub)
			);
		});
	});
	...
	describe('watchDownloadFonts()', () => {
		...
		test('should check the watcher loads up five workers and listens for the font api call', () => {
			...
			for (let i = 0; i < 5; i++) {
				expect(
					gen.next(chan as unknown as Channel<string>).value
				).toEqual(
					fork(getDownloadFonts, chan as unknown as Channel<string>)
				);
			}
	...
	describe('getDownloadFonts()', () => {
		const chanTyped = channel as unknown as Channel<string>;
		const gen = getDownloadFonts(chanTyped);
		const payload = downloadFontsApiCall('test1.ttf') as unknown as string;

		test('should display the pending download message', () => {
			expect(gen.next().value).toEqual(take(chanTyped));
			expect(gen.next(payload).value).toEqual(
				put(addToConsole(payload, 'pending', '[object Object]'))
			);
		});

		test('should display the success download message', () => {
			expect(gen.next().value).toEqual(
				call(api.apiPostDownloadFonts, payload)
			);
			expect(gen.next({ body: 'test', ok: true }).value).toEqual(
				put(addToConsole(payload, 'success', '[object Object]'))
			);
		});

		test('should display the error download message', () => {
			expect(gen.throw(new Error('download failed')).value).toEqual(
				put(addToConsole(payload, 'error', '[object Object]'))
			);
