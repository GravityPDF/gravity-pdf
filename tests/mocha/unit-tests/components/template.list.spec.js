...
import { TemplateList } from '../../../../src/assets/js/components/TemplateList'
...
describe('<TemplateList />', () => {
  ...
  it('our template container, search bar and single template item should be displayed', () => {
    const comp = mount(<Provider store={mockStore( {template: { search: '' }})}>
      <TemplateList templates={Immutable.fromJS([ { id: 'my-id' } ])} route={ { activateText: 'Activate' }}/>
    </Provider>)
    ...
    expect(wrapper.find('.theme')).has.length(1)
  })
