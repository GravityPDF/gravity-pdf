...
class AdvancedCheck {
	constructor() {
		...
		this.debugModeCheckbox = Selector( '#gfpdf-fieldset-debug_mode' ).find(
			'[id="gfpdf_settings[debug_mode]"]'
		);
		this.debugModeErrorMessage = Selector( 'div' ).withText(
			'PDF link not displayed because PDF is inactive.'
		);

		// Merge tags section
		this.mergeTagsButton = Selector( '#gform_setting_message' ).find(
			'button.gform-dropdown__control'
		);
		this.textInputField = Selector( 'input' ).withAttribute(
			'name',
			'input_1'
		);
		this.fNameInputField = Selector( '.name_first' ).find(
			'input[type^="text"]'
		);
		this.lNameInputField = Selector( '.name_last' ).find(
			'input[type^="text"]'
		);
		this.emailInputField = Selector( 'input' ).withAttribute(
			'name',
			'input_3'
		);

		// PDF restriction section
		this.pdfRestrictionErrorMessage = Selector( 'div' )
			.withAttribute( 'class', 'wp-die-message' )
			.withText( 'You do not have access to view this PDF.' );
		this.viewEntryLink = Selector( '.entry_unread' )
			.find( 'a.gravitypdf-download-link' )
			.nth( 1 );
		this.wpLoginForm = Selector( '#login' )
			.find( 'form' )
			.withAttribute( 'name', 'loginform' );

		// Pdf template section
		this.addNewButton = Selector( '.tablenav' )
			.find( 'a' )
			.withText( 'Add New' );
		this.pdfLabelNameInputBox = Selector(
			'#gfpdf-fieldset-gfpdf_form_settings_general'
		).find( '[id="gfpdf_settings[name]"]' );
		this.pdfFilenameInputBox = Selector(
			'#gfpdf-fieldset-gfpdf_form_settings_general'
		).find( '[id="gfpdf_settings[filename]"]' );
		this.addUpdatePdfButton = Selector(
			'#submit-and-promo-container'
		).find( '[id="submit"]' );
		this.templateItem = Selector( '#the-list' ).find( 'tr' );
		this.pdfListSection = Selector( '.gform-settings__navigation' )
			.find( 'a' )
			.withText( 'PDF' );
		this.toggleSwitch = Selector( '#the-list' ).find(
			'.check-column button'
		);
		this.entryItemSection =
			Selector( '#the-list' ).find( 'td.column-primary' );
		this.viewPdfLink = Selector( '#the-list' )
			.find( 'a' )
			.withText( 'View PDF' );
		this.editLink = Selector( '#the-list' )
			.find( 'span' )
			.withText( 'Edit' );
		this.conditionalLogicCheckbox = Selector(
			'#gfpdf-fieldset-gfpdf_form_settings_general'
		).find( '[id="gfpdf_conditional_logic"]' );
		this.templateDetail = Selector( '#the-list' )
			.find( 'tr' )
			.nth( 0 )
			.find( 'td' )
			.nth( 0 );
		this.duplicateLink = Selector( '#the-list' )
			.find( 'a' )
			.withText( 'Duplicate' );
	...
	async copyDownloadShortcode( uri ) {
		...
		await t.click( this.shortcodeBox );
	}

	async navigateConfirmationSection( uri ) {
		await pdf.navigate( uri );

		await t.click( link( '#the-list', 'Default Confirmation' ) );
	}
	...
	async navigateAddPdf( uri ) {
		...
		await t
			...
			.typeText( this.pdfLabelNameInputBox, 'Test PDF Template', {
				paste: true,
			} )
			.typeText( this.pdfFilenameInputBox, 'testpdftemplate', {
				paste: true,
			} )
