...
### 7.0.0
* 🎉 Feature: Gravity PDF now ships with the fonts it needs — Latin, Greek and Cyrillic render out of the box, with true bold and italic, on a fresh install with no internet connection
* ⚠️ Breaking Change: The Core Font installer has been removed. The fonts it downloaded are adopted into the Font Manager during the upgrade, under the same names, so existing PDFs render as they did
* ⚠️ Breaking Change: Fonts dropped straight into the uploads fonts directory are no longer registered automatically on every render. Existing ones are imported into the Font Manager during the upgrade, keeping the name your templates use; add new ones through the Font Manager or `GPDFAPI::add_pdf_font()`
* 🧹 Housekeeping: Fonts are now stored in their own database tables instead of a single plugin option, so nothing on the PDF render path touches the filesystem
* 💻 Developer: `Model_Custom_Fonts::matches_core_font_id()` has been removed, and `Model_Custom_Fonts::__construct()` no longer takes the options class
* 💻 Developer: The font classes have moved from `GFPDF\Helper\Fonts\` to `GFPDF\Fonts\`. The five that shipped in 6.x — `FlushCache`, `LocalFile`, `LocalFilesystem`, `SupportsOtl` and `TtfFontValidation` — are aliased under their old names, so existing code keeps working
* ⚠️ Breaking Change: Remove the version 3 compatibility layer (`src/deprecated.php`), deprecated since Gravity PDF 4.0. The global classes `GFPDF_Core`, `GFPDF_Core_Model`, `GFPDF_Deprecated_Abstract`, `GFPDF_InstallUpdater`, `GFPDF_Notices`, `GFPDF_Settings`, `GFPDF_Settings_Model`, `GFPDFE_DATA` (the class, not the `$gfpdfe_data` global), `GFPDFEntryDetail`, `PDF_Common`, `PDF_Generator`, `PDFGenerator` and `PDFRender` no longer exist, so calling one is a fatal error instead of a deprecation notice
* ⚠️ Breaking Change: Remove the `mPDF` class shim and the `AUTOFONT_ALL` constant it defined. Use `\GFPDF\Helper\Mpdf\Mpdf`
* ⚠️ Breaking Change: Legacy PDF download URLs (`?gf_pdf=1&fid=&lid=&template=`) no longer generate a PDF, and there is no redirect. Nothing handles the request any more, so the link falls through to WordPress and serves the site's front page. Links in notification emails already sent are affected — replace them with the `[gravitypdf]` shortcode or a PDF merge tag. See https://docs.gravitypdf.com/upgrade/legacy-download-urls/
* ⚠️ Breaking Change: Version 3 PDF templates no longer render. A PDF using one fails with an error instead of producing a blank document, and the Legacy group is no longer offered in the template selector. See https://docs.gravitypdf.com/upgrade/legacy-templates/
* ⚠️ Breaking Change: Remove Advanced Templating (Business Plus / Tier 2) support, including the `advanced_template` PDF setting and its Appearance tab behaviour. The setting is ignored. Business Plus / Tier 2 templates are themselves version 3 templates, so those PDFs stop rendering for the reason above rather than falling back to anything
* ⚠️ Breaking Change: Remove the version 3 field classes `Field_V3_List`, `Field_V3_Products` and `Field_V3_Section`, the `Legacy` template configuration class, and the `legacy_css` template configuration option that added version 3 field class names to the PDF markup
* ⚠️ Breaking Change: Remove the `gfpdf_legacy_pre_view_or_download_pdf` action, and the `gfpdf_legacy_save_path`, `gfpdf_legacy_templates` and `gfpdfe_pre_load_template` filters
* ⚠️ Breaking Change: Remove `Model_Settings::check_tmp_pdf_security()`, `::delete_font()`, `::get_font_id_by_name()`, `::install_fonts()`, `::is_font_name_unique()`, `::is_font_name_valid()`, `::process_font()`, `::remove_font_file()`, `::save_font()` and `::test_public_tmp_directory_access()`
* ⚠️ Breaking Change: Remove `Model_Install::deactivate_plugin()`, `::remove_folder_structure()`, `::remove_plugin_form_settings()`, `::remove_plugin_options()` and `::uninstall_plugin()`, plus `Controller_Install::maybe_uninstall()`
* ⚠️ Breaking Change: Remove `Model_PDF::cleanup_pdf()`, `::cleanup_pdf_after_submission()`, `::resend_notification_pdf_cleanup()`, `::get_legacy_config()` and `::handle_legacy_tier_2_processing()`
* ⚠️ Breaking Change: Remove `Helper_Misc::backwards_compat_conversion()`, `::backwards_compat_output()`, `::get_legacy_ids()`, `::maybe_add_multicurrency_support()` and `::maybe_load_gf_entry_detail_class()`
* ⚠️ Breaking Change: Remove `Helper_Notices::maybe_remove_non_pdf_messages()`, `::reset_gravityforms_messages()`, `::set_gravitypdf_errors()` and `::set_gravitypdf_notices()`
* ⚠️ Breaking Change: Remove `Controller_PDF::process_legacy_pdf_endpoint()` and `::sgoptimizer_html_minification_fix()`, `Controller_Pdf_Queue::queue_cleanup_task()` and `::queue_async_resend_notification_tasks()`, and `Queue_Callbacks::cleanup_pdfs()`
* ⚠️ Breaking Change: Remove `View_PDF::display_page_name()` and `::get_template_filename()`, and `View_Settings::tabs()`
* 💻 Developer: `View_PDF::generate_pdf()`, deprecated in 6.12, now hands the whole job to `\GPDFAPI::create_pdf()` and streams what comes back, instead of driving `Helper_PDF` itself. It is cached and runs the current hooks, so it produces the same document every other render does
* ⚠️ Breaking Change: Remove `Model_Shortcodes::gravitypdf()` (the 5.2 alias of `::process()` — the `[gravitypdf]` shortcode itself is unaffected), `Helper_Options_Fields::get_advanced_template_field()` and `Helper_Abstract_Options::get_font_display_name()`
* ⚠️ Breaking Change: Remove the two no-op `maybe_display_canonical_plugin_notice*` stubs from `pdf.php`, deprecated in 6.12
* 💻 Developer: The `$legacy_ids` argument to `Helper_Templates::get_template_arguments()` is deprecated. It is still accepted, so a caller on the old signature keeps working, but it is no longer read: the multi-entry legacy endpoint was the only thing that had more than one entry to name, so `lead_ids` now always reports the entry being rendered
* ⚠️ Breaking Change: Delete the `PDF/page_title.php` view partial
* 🧹 Housekeeping: Rename the `Settings/tabs.php` view partial to `Settings/sub_menu.php`, matching the method that renders it now `View_Settings::tabs()` is gone
* ⚠️ Breaking Change: Remove the `$args['content']` fallback from the settings, form settings and Gravity Forms views, deprecated in 6.4/6.5. Pass `$args['callback']` instead
* ⚠️ Breaking Change: Remove the 6.17 legacy endpoint usage recorder — `Deprecation_V3::record_legacy_endpoint_usage()` and `::get_recorded_legacy_endpoint_usage()` — along with `Deprecation_V3::has_tier_2_addon()`, `::is_advanced_template_pdf()`, `::restore_v3_form_class()` and the `LEGACY_ENDPOINT_TTL`, `LEGACY_ENDPOINT_REFRESH`, `TIER_2_ADDON_CLASS` and `INTERNAL_FILTER_CALLBACK` constants
* ⚠️ Breaking Change: Remove the version 3 filters `gfpdfe_template_location`, `gfpdfe_template_location_uri`, `gfpdfe_pdf_output_type`, `gfpdfe_pdf_name`, `gfpdfe_template`, `gfpdfe_pdf_filename`, `gfpdfe_pdf_template`, `gfpdfe_pdf_template_{form_id}`, `gfpdfe_mpdf_class_pre_render`, `gfpdfe_pre_render_pdf`, `gfpdfe_mpdf_class`, `gfpdfe_lead_id` and `gfpdfe_signature_width`, deprecated since 4.0. A callback attached to one of these stops running, and does so silently. Use `gfpdf_template_location`, `gfpdf_template_location_uri`, `gfpdf_pdf_config`, `gfpdf_pdf_filename`, `gfpdf_pdf_html_output`, `gfpdf_pdf_html_output_{form_id}`, `gfpdf_mpdf_class`, `gfpdf_template_args` and `gfpdf_signature_width` instead; `gfpdfe_pdf_output_type` has no replacement. See https://docs.gravitypdf.com/upgrade/deprecated-filters/
* ⚠️ Breaking Change: `gfpdfe_template_location` and `gfpdfe_template_location_uri` ran **before** their `gfpdf_` counterparts, so a site hooking both had the version 3 result handed to the version 4 filter. With the version 3 pair gone, `gfpdf_template_location` and `gfpdf_template_location_uri` receive the plugin's own default instead
* ⚠️ Breaking Change: Remove the version 3-shaped aliases `gfpdf_orientation`, `gfpdf_security`, `gfpdf_privilages` (spelt as it always was), `gfpdf_password`, `gfpdf_master_password` and `gfpdf_rtl`, along with `Model_PDF::apply_backwards_compatibility_filters()`, which fired them, and `Helper_Misc::update_deprecated_config()`, which converted their boolean returns. Use `gfpdf_pdf_config`
* 🧹 Housekeeping: The Actions and Filters detection moves from Deprecated Features to Unsupported Features along with the rest of the version 3 detections. A site with a callback still attached to one of the removed hooks is told the callback has stopped running, rather than that it is scheduled to. Since nothing declares deprecated-but-working functionality any more, the Deprecated Features section no longer appears at all
* 💻 Developer: `PDF_SAVE_LOCATION`, `PDF_FONT_LOCATION`, `PDF_TEMPLATE_LOCATION`, `PDF_TEMPLATE_URL_LOCATION` and the `$gfpdfe_data` global are **not** removed — PDF templates still rely on them, and they remain supported. What changed is when they appear: they are defined once a PDF render begins, rather than on every request, and then persist for the rest of that request. Code that read one before any PDF render (at `init`, in an admin screen, in another plugin) must switch to `GPDFAPI::get_data_class()` or `GPDFAPI::get_templates_class()`
* 💻 Developer: `Helper_Abstract_Options::get_custom_fonts()` and `::get_font_short_name()` are no longer marked `@deprecated`. Both are used internally and the tag was incorrect
* 🐛 Bug: A select whose stored value is no longer in its option list now shows that value, flagged as not currently available, instead of falling back to whatever sorts first. A PDF still set to a version 3 template is the case that matters in 7.0: the template selector stopped offering those, so opening the PDF and saving it would have silently switched the PDF to another template. The same applies to any select, such as a PDF using a font that has since been deleted, or a template that needs a newer version of Gravity PDF than the site is running
...
* 💻 Developer: `Helper_Data::get_localised_script_data()` takes a third required `Helper_Templates` argument, and no longer reads `PDF_TEMPLATE_LOCATION`
