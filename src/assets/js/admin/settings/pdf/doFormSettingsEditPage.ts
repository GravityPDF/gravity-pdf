...
export function doFormSettingsEditPage(): void {
	...
	toggleFontAppearance(
		String($('#gfpdf_settings\\[template\\]').data('template_group') ?? '')
	);
	toggleAppearanceTab();
	handleMergeTags();
	...
	if (
		$templateSection.find('.gform-settings-panel__content').children()
			.length === 0
	) {
	...
	$('#gfpdf_pdf_form').on('submit', function () {
		...
		if (window.gfpdf_current_pdf !== undefined) {
			$('#gfpdf_settings\\[conditionalLogic\\]').val(
				$.toJSON(window.gfpdf_current_pdf.conditionalLogic)
			);
		}
	});
}
