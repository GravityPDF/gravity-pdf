...
export function handlePDFConditionalLogic() {
	...
	gform.addFilter('gform_conditional_logic_fields', function (options) {
		for (const property in entryOptions) {
			...
			if (
				Object.hasOwn(entryOptions, property) &&
				!options.find(
					(opt) => opt.value === entryOptions[property].value
				)
			) {
				options.push({
			...
			if (Object.hasOwn(entryOptions, selectedFieldId)) {
				if (entryOptions[selectedFieldId].choices) {
					...
					str = GetRuleValuesDropDown(
						entryOptions[selectedFieldId].choices,
						objectType,
						ruleIndex,
						selectedValue,
						inputName
				...
				if (entryOptions[selectedFieldId].placeholder) {
					str = $(str).attr(
						'placeholder',
						entryOptions[selectedFieldId].placeholder
					)[0].outerHTML;
				}
			}
