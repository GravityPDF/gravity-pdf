...
export function snapshotFormValues ($container) {
  ...
  $container.find(':input').not(TEMPLATE_DROPDOWN_SELECTOR).each(function () {
    const $el = $(this)
    ...
    if ($el.is(':checkbox') || $el.is(':radio')) {
      entries.push({ kind: 'input', name, value: $el.val(), checked: $el.prop('checked') })
      return
    }
    ...
    if ($el.is('textarea') && id && typeof tinyMCE !== 'undefined') {
      ...
      if (editor && !editor.hidden) {
        entries.push({ kind: 'input', name, value: editor.getContent() })
        return
      }
    }
  ...
  $container.find('.gfpdf-input-toggle').each(function () {
    const $toggle = $(this)
    const controlsName = $toggle.parent().next().find('textarea').first().attr('name')
    if (!controlsName) {
      return
    }
    entries.push({ kind: 'toggle', controls: controlsName, checked: $toggle.prop('checked') })
  })
...
export function restoreFormValues ($container, snapshot) {
  snapshot.forEach(function (item) {
    if (item.kind === 'toggle') {
      const $toggle = $container.find('.gfpdf-input-toggle').filter(function () {
        return $(this).parent().next().find('textarea').first().attr('name') === item.controls
      })
