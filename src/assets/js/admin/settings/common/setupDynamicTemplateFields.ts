...
declare function gform_initialize_tooltips(): void;
/* eslint-enable no-var */

interface TemplateFieldsResponse {
	fields?: string;
	editors?: string[];
	editor_init?: Record<string, unknown>;
 ...
 * @since 4.0
 */
export function setupDynamicTemplateFields(): void {
	...
	$('#gfpdf_settings\\[template\\]')
		...
		.on('change', function (this: HTMLElement) {
			...
			ajaxCall(data, function (rawResponse: unknown) {
				const response = rawResponse as TemplateFieldsResponse;
