			...
			function GravityPDF () {
			   ...
			   *
				...
				this.handlePDFConditionalLogic = function () {
					...
					$( '#gfpdf_conditional_logic' ).change(
						function () {
								...
								self.ajax(
									...
									function (response) {
										if (response.msg) {
											...
											$newRow.find( '.column-name > a, .edit a' ).each(
												function () {
							...
							self.ajax(
								...
								function (response) {
									if (response.msg) {
										...
										$row.css( 'background', '#ffb8b8' ).fadeOut(
											400,
											function () {
												this.remove()
			   ...
			   *
				...
				this.setupToggledFields = function () {
					...
					$( 'form' ).off( 'change', '.gfpdf-input-toggle' ).on(
						'change',
						'.gfpdf-input-toggle',
						function () {
							...
							if ($( this ).prop( 'checked' )) {
								...
								$container.find( '.wp-editor-area' ).each(
									function () {
			   ...
			   *
				...
				this.setupDynamicTemplateFields = function () {
					...
					$( '#gfpdf_settings\\[template\\]' ).off( 'change' ).change(
						function () {
			   ...
			   *
				...
				this.setupLicenseDeactivation = function () {
					$( '.gfpdf-deactivate-license' ).click(
						function () {
			   ...
			   *
				...
				this.toggleAppearanceTab = function () {
					...
					$( 'input[name="gfpdf_settings[advanced_template]"]' ).change(
						function () {
							if ($( this ).val() == 'Yes') {
			   ...
			   *
				...
				this.doUploadListener = function () {
					...
					$( 'body' ).off( 'click', '.gfpdf_settings_upload_button' ).on(
						'click',
			   ...
			   *
				...
				this.setupRequiredFields = function ($elm) {
					...
					$elm.find( 'tr input[type="submit"]' ).click(
						function () {
					...
					*/
					$advanced_options.click(
						function () {
							...
							/* toggle our slider */
							$advanced_options_container.slideToggle(
								600,
								function () {
