...
export class AddUpdateFontFooter extends Component {
	...
	render() {
		...
		const errorFontValidation =
			...
			error.fontValidationError;
		// eslint can't detect %s on fontFileMissing
		// eslint-disable-next-line @wordpress/valid-sprintf
		const fontFileMissing = sprintf(
					...
					<div className="select-delete-icons-container">
						{id && (
							<button
								...
								onClick={() =>
									this.handleSelectFont(id, selectedFont)
								}
								onKeyDown={(e) =>
									this.handleSelectFontKeypress(
										e,
										id,
										selectedFont
									)
						...
						{id && (
							<button
								...
								onKeyDown={(e) =>
									this.handleDeleteFontKeypress(e, id)
								}
								type="button"
								tabIndex={tabIndex}
								aria-label={
									GFPDF.fontManagerDeleteFontAriaLabel
								}
							/>
						)}
					</div>
 ...
 *
 * @return {{ selectedFont: string }} mappedState
 *
 * @since 6.0
 */
