 ...
 *
 * @param {Object} schema
 * @return {FormData} Serialised PDF settings.
 *
 * @since 7.0
 */
...
gform.addFilter('gfpdf_preview_settings', (formData, schema) => {
	...
	if (
		!schema.properties.custom_pdf_size ||
		formData.get('pdf_size') !== 'CUSTOM'
	) {
		return formData;
	}

	formData.set(
		'custom_pdf_size[width]',
		document.getElementById('gfpdf_settings[custom_pdf_size]_width').value
	);
	formData.set(
		'custom_pdf_size[height]',
		document.getElementById('gfpdf_settings[custom_pdf_size]_height').value
	);
	formData.set(
		'custom_pdf_size[unit]',
		document
			.getElementById('gfpdf_settings[custom_pdf_size]_measurement')
			.value.replace('millimeters', 'mm')
			.replace('inches', 'in')
	);
 ...
 */
// eslint-disable-next-line no-unused-vars -- filter signature requires schema arg.
gform.addFilter('gfpdf_preview_settings', (formData, schema) => {
