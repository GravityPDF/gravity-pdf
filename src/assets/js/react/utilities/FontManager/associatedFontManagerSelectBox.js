...
export function associatedFontManagerSelectBox(fontList, id) {
	...
	const selectedValue =
		fontManagerSelectBox.options[fontManagerSelectBox.selectedIndex].value;
	...
	Array.from(
		document.querySelectorAll(
			'optgroup[label="User-Defined Fonts"] > option'
		)
	).map((item) => userDefinedFonts.push(item.value));
	...
	if (userDefinedFonts.length === 0 && list.length === 0) {
		fontManagerSelectBox
			.querySelector('optgroup[label="User-Defined Fonts"]')
			.remove();
