...
class Helper_Templates {
	...
	public function get_all_templates_by_group() {
		...
		foreach ( $template_list as $template_path ) {
			...
			$template_groups[ $info['group'] ][ $info['id'] ] = $this->maybe_add_template_compatibility_notice( $info['template'], $info['required_pdf_version'] );
		}
	...
	public function maybe_add_template_compatibility_notice( $template_name, $required_version ) {
		if ( ! $this->is_template_compatible( $required_version ) ) {
			return $template_name . ' (+ ' . esc_html_x( 'needs', 'Required', 'gravity-forms-pdf-extended' ) . ' v' . $required_version . ')';
		}
	 ...
	 * @since 4.1
	 */
	public function get_template_path_by_id( $template_id ) {
	...
	public function get_template_config_file( $template ) {
		...
		foreach ( $test_config_paths as $path ) {
			...
			try {
				$class = $this->load_template_config_file( $file );
			} catch ( Exception $e ) {
				$this->log->addNotice( $e->getMessage(), [ 'file' => $file ] );
			}
	...
	public function get_template_image( $template ) {
		...
		$test_image_paths = [
			...
			PDF_PLUGIN_URL . 'src/templates/images/'       => PDF_PLUGIN_DIR . 'src/templates/images/',
		];
