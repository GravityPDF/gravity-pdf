...
class Catalog_Repository {
	 ...
	 *
	 * Identity by the `package` column, which is the name the source published for its own archive (§4.3 Hosting).
	 * Not the embedded `package.path`, which would answer for the 17 packs and for nothing else — a display entry
	 * carries a pointer rather than an inlined entry object — and not a filename this rebuilt from `entry` and
	 * `version`, which would put the pipeline's naming grammar in two repositories with nothing gating them equal.
	 *
	 * Uncached on purpose: one read per upload, a hand-driven admin action. The row it names is then read through
	 * `entry()`, which is cached.
	 *
	 * An empty name matches nothing rather than matching the rows that published no archive: `upsert()` binds every
	 * column as `%s`, so those hold `''` and not NULL.
	 *
	 * @since 7.0
	 */
	public function entry_for_package( string $name ): ?array {
		...
		$row = $wpdb->get_row( $wpdb->prepare( "SELECT source, entry FROM {$table} WHERE package = %s", $name ), ARRAY_A );
