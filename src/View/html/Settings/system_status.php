...
<table id="pdf-system-status" class="form-table">
	<tr>
		...
		<td>
			...
			<?php if ( $args['memory'] === -1 ): ?>
				<?= esc_html__( 'Unlimited', 'gravity-forms-pdf-extended' ); ?>
			<?php else : ?>
				<?= $args['memory']; ?>MB
			<?php endif; ?>

			<span class="<?= $ram_icon; ?>"></span>

			<?php if ( $args['memory'] < 128 && $args['memory'] !== -1 ): ?>
				<span class="gf_settings_description">
					<?= sprintf( esc_html__( 'We strongly recommend you have at least 128MB of available WP Memory (RAM) assigned to your website. %1$sFind out how to increase this limit%2$s.', 'gravity-forms-pdf-extended' ), '<br /><a href="https://gravitypdf.com/documentation/v5/user-increasing-memory-limit/">', '</a>' ); ?>
				</span>
			<?php endif; ?>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			...
			<span class="<?= $allow_url_fopen_icon; ?>"></span>

			<?php if ( ! $args['allow_url'] ): ?>
				<span class="gf_settings_description">
					<?= sprintf( esc_html__( 'We detected the PHP runtime configuration setting %1$sallow_url_fopen%2$s is disabled.', 'gravity-forms-pdf-extended' ), '<a href="https://www.php.net/manual/en/filesystem.configuration.php#ini.allow-url-fopen"><code>', '</code></a>' ); ?> <br>
					<?= esc_html__( 'You may notice image display issues in your PDFs. Contact your web hosting provider for assistance enabling this feature.', 'gravity-forms-pdf-extended' ); ?>
				</span>
			<?php endif; ?>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			...
			<div id="gfpdf-direct-pdf-protection-check" data-nonce="<?= wp_create_nonce( 'gfpdf-direct-pdf-protection' ); ?>">
